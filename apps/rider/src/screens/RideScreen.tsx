...
const STATUS_COLORS: Record<RideStatus, string> = {
  searching: '#f59e0b', accepted: '#3b82f6', enroute: '#10b981',
  picked_up: '#10b981', in_progress: '#10b981', completed: '#64748b', cancelled: '#ef4444',
};
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0a1628' }}>
      <MapView style={{ flex: 1 }} mapStyle={MAP_STYLE} logoEnabled logoPosition={{ bottom: 8, left: 8 }}>
        <Camera
          defaultSettings={{ centerCoordinate: [-74.006, 40.7128], zoom: 14, pitch: 45 }}
        />
      ...
      {status !== 'searching' && status !== 'cancelled' && (
        <View style={styles.rideCard}>
          ...
          <View style={styles.rideStats}>
            <View style={styles.stat}><Text style={styles.statLabel}>ETA</Text><Text style={styles.statValue}>3 min</Text></View>
            <View style={styles.statDivider} />
            <View style={styles.stat}><Text style={styles.statLabel}>Fare</Text><Text style={styles.statValue}>${fare.toFixed(2)}</Text></View>
            <View style={styles.statDivider} />
            <View style={styles.stat}>
              <Text style={styles.statLabel}>Status</Text>
              <Text style={[styles.statValue, { fontSize: 13, color: STATUS_COLORS[status] }]}>{statusLabel(status)}</Text>
            </View>
          </View>
          <View style={styles.actions}>
            <TouchableOpacity style={styles.actionBtn} onPress={() => Alert.alert('Trip shared', 'Your location has been shared.')}>
              <Ionicons name="share-outline" size={18} color="#475569" /><Text style={styles.actionText}>Share</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.actionBtn, styles.actionDanger]} onPress={cancelRide}>
              <Ionicons name="close-outline" size={18} color="#dc2626" /><Text style={[styles.actionText, { color: '#dc2626' }]}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.actionBtn} onPress={() => { Alert.alert('Safety Alert', 'Contacts notified.'); Vibration.vibrate(200); }}>
              <Ionicons name="shield-outline" size={18} color="#dc2626" /><Text style={[styles.actionText, { color: '#dc2626' }]}>Safety</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
...
const styles = StyleSheet.create({
  topBar: {
    ...
    padding: 12, backgroundColor: 'rgba(10,22,40,0.9)',
  },
  backBtn: { padding: 4 },
  ...
  searchingOverlay: {
    ...
    backgroundColor: 'rgba(10,22,40,0.7)', zIndex: 5,
  },
  searchingCircle: {
  ...
  avatar: { width: 48, height: 48, borderRadius: 14, backgroundColor: '#c9952b', justifyContent: 'center', alignItems: 'center' },
  ...
  rideStats: {
    ...
    borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#e2e8f0', marginBottom: 12,
  },
  ...
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 12, borderRadius: 12, backgroundColor: '#f1f5f9' },
